...
export default function Logo3D({ size = 200, className = "", animate = true }: Logo3DProps) {
  const loopEase = cubicBezier(0.4, 0, 0.2, 1);
  const svgStyle: CSSProperties = {
    filter: "drop-shadow(0 0 25px rgba(250, 204, 21, 0.6))",
    transformStyle: "preserve-3d",
  };

  const content = (AnimatedGroup: typeof motion.g | "g"): ReactNode => (
    <>
      ...
      <AnimatedGroup
        {...(animate
          ? {
              ...
              transition: { duration: 6, repeat: Infinity, ease: loopEase },
            }
          : {})}
      >
        <polygon
      ...
      <AnimatedGroup
        {...(animate
          ? {
              ...
              transition: { duration: 2, repeat: Infinity, ease: loopEase },
            }
          : {})}
      >
        <circle
