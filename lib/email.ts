...
const RESEND_FROM = "Multicell System <suporte@multicellsystem.com.br>";
  ...
  <div style="background:#040814;padding:34px;font-family:Arial,sans-serif;color:#e5e7eb;">
    <div style="max-width:580px;margin:0 auto;background:linear-gradient(160deg,rgba(17,24,39,0.95),rgba(15,23,42,0.92));border:1px solid rgba(250,204,21,0.36);border-radius:20px;padding:30px;box-shadow:0 20px 70px rgba(250,204,21,0.14);">
      <div style="text-align:center;margin-bottom:22px;">
        <img src="https://multicellsystem.com.br/logo.png" alt="Multicell" style="height:60px;object-fit:contain;filter:drop-shadow(0 0 14px rgba(250,204,21,0.3));" />
      </div>
...
const buildSubscriptionCancelledTemplate = (email: string) => {
  ...
  <div style="background:#040814;padding:34px;font-family:Arial,sans-serif;color:#e5e7eb;">
    <div style="max-width:580px;margin:0 auto;background:linear-gradient(160deg,rgba(17,24,39,0.95),rgba(15,23,42,0.92));border:1px solid rgba(250,204,21,0.36);border-radius:20px;padding:30px;box-shadow:0 20px 70px rgba(250,204,21,0.14);">
      ...
      <p style="margin:0;color:#94a3b8;line-height:1.6;">Se quiser voltar ao ecossistema Multicell, basta realizar um novo pagamento para reativar o acesso.</p>
    </div>
  </div>
  `;
};
...
export async function sendPasswordResetEmail(params: {
  ...
  const response = await resend.emails.send({
    from: RESEND_FROM,
    to: params.to,
    subject: "Multicell - Recuperação de Senha",
...
export async function sendSubscriptionCancelledEmail(params: {
  ...
  const response = await resend.emails.send({
    from: RESEND_FROM,
    to: params.to,
    subject: "Multicell - Assinatura cancelada",
